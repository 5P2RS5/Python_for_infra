...
if [ -z "$3" ]; then
    ...
    # 폴더가 없을 때
    if [ ! -d "$PWD/programmers/$3" ] ; then
        echo "폴더가 없습니다." $3"폴더를 생성합니다 🗂️"
        mkdir $PWD/programmers/$3
    fi

    if [ -f "$PWD/programmers/$3/$1.py" ] ; then
        echo "[$1 : $2]는 이미 풀었어!!, 또 풀어볼래?"
        read answer
        if [ "$answer" == "y" ] || [ "$answer" == "Y" ]; then
            echo "[$1 : $2]를 삭제할게 😅"
            rm "$PWD/programmers/$3/$1.py"
        else
            echo "OK, 다음 문제로 넘어가자 🏃‍♂️"
            exit
        fi
